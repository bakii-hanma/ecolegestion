...
class StatisticsController extends Controller
{
    public function index()
    {
        ...
        if ($currentYear) {
            ...
            $totalRevenue = OnlinePayment::sum('amount');
            $monthlyRevenue = OnlinePayment::whereMonth('created_at', now()->month)
                ->whereYear('created_at', now()->year)
                ->sum('amount');
            $paymentsCompleted = OnlinePayment::where('status', 'completed')->count();
            $paymentsPending = OnlinePayment::where('status', 'pending')->count();
            $paymentsCancelled = OnlinePayment::where('status', 'cancelled')->count();
        }
